...
3. **② 임베딩 시작**: 최초 1회 임베딩 모델을 자동으로 내려받아 브라우저에 캐시합니다(인터넷 필요).
4. **③ 검색**: 예) `임산부 시간외근로`, `전용통신망 수의계약` → 관련도 순으로 결과 표시.

- 문서 내용은 **브라우저 밖으로 전송되지 않습니다**(완전 로컬).
- HWP·HWPX·PDF 추출과 임베딩·검색이 모두 페이지 안에서 동작합니다.
- 임베딩하면 **이 브라우저에 자동 저장**(IndexedDB)됩니다 — 페이지를 다시 열면 재임베딩 없이 바로 검색. `저장된 인덱스 지우기`로 삭제할 수 있습니다.
- `인덱스 저장(.json)` 으로 다른 PC에 옮길 파일로도 보관하고, `인덱스 불러오기`로 즉시 검색할 수 있습니다.
- **모델을 페이지에서 고를 수 있습니다**: `e5-small`(빠름·저품질) · `e5-base`(권장·균형) ·
  `e5-large`(품질 최고·느림, ~1.2GB). 큰 모델일수록 의미 이해가 좋아집니다. 모델을 바꾸면 다시 임베딩하세요.
- 관련도 낮은(무관) 결과는 자동으로 **숨깁니다.** 단, 브라우저 모델은 파이썬 경로(KURE-v1)보다
  의미 이해가 낮으므로, 정밀한 한국어 문맥검색은 파이썬 경로를 권장합니다.
