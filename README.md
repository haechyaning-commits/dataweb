...
3. **② 임베딩 시작**: 최초 1회 임베딩 모델을 자동으로 내려받아 브라우저에 캐시합니다(인터넷 필요). BGE-M3는 고성능 모델이라 처음 다운로드에 수 분이 걸릴 수 있으나, 이후에는 캐시에서 즉시 로드됩니다.
4. **③ 검색**: 예) `임산부 시간외근로`, `전용통신망 수의계약` → **문맥상 관련 있는 문서만** 유사도 순으로 표시.

- 문서 내용은 **브라우저 밖으로 전송되지 않습니다**(완전 로컬).
- HWP·HWPX·PDF 추출과 임베딩·검색이 모두 페이지 안에서 동작합니다.
- **하이브리드 검색**: 임베딩 기반 **의미검색(dense)** 과 **키워드 정확일치(BM25)** 를 융합(의미 60% + 키워드 40%)해 순위를 매깁니다. `제74조`·`케이티`·`990,323원` 같은 정확 용어도 놓치지 않습니다. 검색창 아래 세그먼트로 `하이브리드 / 의미검색 / 키워드` 를 전환하며 비교할 수 있습니다.
- **문맥 기반 필터링**: 선택한 개수(Top-K)는 상한일 뿐이며, 유사도 임계치를 넘지 못하는 문서는 자동으로 제외됩니다. 임계치를 넘는 문서가 없으면 "문맥적으로 일치하는 문서가 없습니다" 안내가 표시됩니다.
- 검색어 키워드는 본문/프리뷰에서 **노란색으로 하이라이팅**되며, 정확히 일치한 용어는 **키워드 칩**으로 표시됩니다.
- `인덱스 저장(.json)` 으로 임베딩 결과를 파일로 보관하고, 다음에 `인덱스 불러오기`로 즉시 검색할 수 있습니다.
- 모델: `Xenova/bge-m3`(다국어·한국어 특화, 문맥 이해 우수, 긴 문맥 지원).
